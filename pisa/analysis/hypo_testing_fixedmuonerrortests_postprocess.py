...
the `hypo_testing_fixedmuonerrortests.py` script. That is, give an Asimov 
sensitivity for 3 different values of the fixed muon errors.
...
def extract_trials(logdir, fluctuate_fid, fluctuate_data=False):
    ...
    for folder in logdir_content:
        if '.pckl' not in folder and 'Plots' not in folder:
            ...
            h0_error_vals.append(folder.split('start')[1].split('_')[1])
            h1_error_vals.append(folder.split('start')[2].split('_')[1])
    toy_names = np.array(toy_names)
    h0_error_vals = np.array(h0_error_vals)
    h1_error_vals = np.array(h1_error_vals)
    ...
    if not np.alltrue(h0_error_vals == h1_error_vals):
        raise ValueError('Not all output matches the muon error calculation '
                         'point for both hypotheses.')    
    ...
    if 'data_sets.pckl' in logdir_content:
        ...
        for outputdir in logdir_content:
            ...
            if 'config_summary.json' in outputdir_content:
                ...
                # Temporary correction because of typo in original script
                if 'dimensionless' in cfg['h1_name']:
                    cfg['h1_name'] = cfg[
                        'h1_name'].split('_')[0] + \
                        cfg['h0_name'].split(cfg[
                            'h0_name'].split('_')[0])[-1]
                # Get naming scheme
                labels = Labels(
                ...
                errorpoint = labels.dict['h0_name'].split('_')[-1]
                all_labels[errorpoint] = labels
                ...
                all_params[errorpoint] = these_params
...
def plot_significance(error_points, significances, truth):
    '''
    ...
    plt.xlabel('%s Value for Error Calculation Point'
               %tex_axis_label('atm_muon_scale'))
    plt.ylabel(r'Significance ($\sigma$)')


def plot_significances(WO_to_TO_metrics, TO_to_WO_metrics, error_points,
                       labels, detector, selection, outdir):
    '''
    Takes the two sets of metrics relevant to the Asimov-based analysis and 
    makes a plot of the significance for all of the error points.
    '''
    ...
    injlabels = labels['%.4f'%(error_points[0])].dict
    ...
    plt.title(MainTitle,fontsize=16,y=1.05)
    plt.axhline(
        significances[1],
        linestyle='--',
        linewidth=2,
    ...
    plt.legend(loc='best')
    SaveName = "true_%s_%s_%s_muon_error_points_asimov_significances.png"%(
        truth,
        detector,
        selection
    )
...
def plot_fit(error_points, fit_param,
...
def plot_individual_fits(WO_to_TO_params, TO_to_WO_params, error_points,
                         labels, detector, selection, outdir):
    '''
    Takes the two sets of best fit parameters relevant to the Asimov-based 
    analysis and makes plots of them as a function of the error points.
    ...
    injlabels = labels['%.4f'%error_points[0]].dict
    truth = injlabels['data_name'].split('_')[0]
    h0 = injlabels['h0'].split('hypo_')[-1].split('_')[0]
    h1 = injlabels['h1'].split('hypo_')[-1].split('_')[0]
    if h0 == truth:
        TO = h0
        WO = h1
    else:
    ...
    for param in WO_to_TO_params.keys():
        ...
        SaveName = "true_%s_%s_%s_muon_error_points_%s_best_fit_values.png"%(
            truth,
            detector,
            selection,
            param
...
def plot_combined_fits(WO_to_TO_params, TO_to_WO_params, error_points,
                       labels, detector, selection, outdir):
    '''
    Takes the two sets of best fit parameters relevant to the Asimov-based 
    analysis and makes plots of them as a function of the error calculation 
    point. This will use plot_fit and save once one for each parameter is on 
    the canvas.
    '''
    ...
    SaveName = "true_%s_%s_%s_muon_error_points_all_best_fit_values.png"%(
        truth,
        detector,
        selection
    )
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="""Directory into which the output of 
        hypo_testing_fixedmuonerrortests.py was stored."""
    )
    parser.add_argument(
...
def main():
    ...
    error_points = []
    for error_point in data_sets.keys():
        error_points.append(float(error_point))
