...
import pisa.analysis.llr.LLHAnalysis as LLHAnalysis
import pisa.analysis.stats.Maps as Maps
from pisa.analysis.TemplateMaker import TemplateMaker
from pisa.utils.params import get_values, select_hierarchy

parser = ArgumentParser(
    description='Runs the Asimov optimizer-based analysis varying a number of'
    ' systematic parameters defined in settings.json file and saves the'
    ' likelihood (or chisquare) values for all combinations of hierarchies.',
...
parser.add_argument(
    '-c', '--chisquare',
    action='store_true', default=False, dest='use_chisquare',
    help='Use chisquare metric instead of log-likelihood.'
)
parser.add_argument(
    '--check-octant',
    action='store_true', default=False, dest='check_octant',
    help='After first optimisation, check whether seeding with theta23 mirrored'
    ' into the alternative octant leads to a better solution (requires 2x the'
    ' amount of time).'
)
parser.add_argument(
    '-pd', '--pseudo_data_settings',
    type=str, metavar='JSONFILE', default=None,
...
for data_tag, data_normal in [('data_NMH',True)]: #, ('data_IMH',False)]:
    ...
    asimov_fmap = Maps.get_asimov_fmap(
        pseudo_data_template_maker,
        get_values(select_hierarchy(pseudo_data_settings['params'], normal_hierarchy=data_normal)),
        channel=channel
    )
    ...
    for hypo_tag, hypo_normal in [('hypo_NMH',True), ('hypo_IMH',False)]:
        ...
        opt_data = LLHAnalysis.find_opt_scipy(
            asimov_fmap,
            template_maker,
