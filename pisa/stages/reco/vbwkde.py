...
__all__ = ['EPSILON', 'KDE_DIM_DEP', 'KDE_TRUE_BINNING', 'MIN_NUM_EVENTS',
           ...
           'collect_enough_events', 'fold_coszen_err', 'extract_coordinate',
           'vbwkde',
           'plot_kde_detail', 'plot_multiple']
...
def extract_coordinate(index, dim_lengths):
    """
    Parameters
    ----------
    index : int
        Zero-indexed index into the sequence generated by
    ...
    def characterize_resolutions(self):
        ...
        for kde_dim, dependencies in self.kde_dim_dep.items():
            ...
            for dep in dependencies:
                dep_dim = self.kde_binning[dep]
                dep_dims.append(dep_dim)
                augmented_hash_items.append(hash(dep_dim))
                dim_lengths.append(len(dep_dim))
            ...
            for bin_num, bin_dims in enumerate(product(*dep_dims)):
                bin_coord = extract_coordinate(bin_num, dim_lengths)

                # Cut on all but the final dimension
                ...
                for flavintgroup in self.transform_groups:
                    ...
                    if kde_dim == 'pid':
                        ...
                        # Make mirrored copies above/below +/-1 to minimize
                        # KDE edge effects. The result will have to have just
                        # the center section removed and be re-normalized to
                        # have an area of one.
                        ...
                        raise NotImplementedError(
                            'Do not know how to apply KDEs to the %s'
                            ' dimension.' % kde_dim
                        )

                    _, x, density = vbw_kde(feature, n_dct=2**14)
