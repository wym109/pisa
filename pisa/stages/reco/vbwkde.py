...
KDE_TRUE_BINNING = MultiDimBinning([
    dict(name='true_energy', num_bins=20, is_log=True, domain=[1, 80]*ureg.GeV),
...
def extract_coordinate(flat_index, dim_lengths):
    """
    Parameters
    ----------
    flat_index : int
        Flat index: starting at zero, indexes into the sequence generated by
        `product(dim_0, dim_1, ..., dim_N)`.

    dim_lengths : Iterable of integers
        Lengths of each dimension

    Returns
    -------
    coordinate : list of integer
        Coordinate e.g. [x_index, y_index, z_index]. Note that all indices are
        0-indexed.

    """
    if not isinstance(dim_lengths, Sequence):
        dim_lengths = list(dim_lengths)
    coord = []
    q = flat_index
    for dim_length in dim_lengths[::-1]:
        q, r = divmod(q, dim_length)
        coord.append(r)
    return coord[::-1]
...
class vbwkde(Stage):
    ...
    def characterize_resolutions(self):
        ...
        """
        # TODO: add sourcecode hash for vbwkde module (entire module is
        #       probably safest, due to all the functions there)

        # Only hash on the things that could be relevant to the KDE's:
        # 1. source code: if the code changes at all
        # 2. reco_events: event used to compute the transforms + cuts applied
        #       (also implicitly includes flav/ints grouped together)
        # 3. kde_binning / output_binning.pid: only the dimension(s) of
        #       kde_binning and possibly output_binning.pid. This is attached
        ...
        for kde_dim, dependencies in self.kde_dim_dependencies.items():
            ...
            dep_dims = []
            dim_lengths = []
            for dep_dim_name in dependencies:
                dep_dim_binning = self.kde_binning[dep_dim_name]
                dep_dims.append(dep_dim_binning)
                augmented_hash_items.append(hash_obj(dep_dim_binning))
                dim_lengths.append(len(dep_dim_binning))

            new_hash = hash_obj(augmented_hash_items)
            ...
            cut_events = [self.events] * (1 + len(dep_dims))

            for bin_num, bin_dims in enumerate(product(*dep_dims)):
                bin_coord = self.kde_binning.flatindex2coord(bin_num)

                # Apply cuts for all but the last dimension dependency (which
                # is expected to be true_energy, for now)
                ...
                for flavintgroup in self.transform_groups:
                    ...
                    if kde_dim == 'pid':
                        ...
                        vbwkde_kwargs = dict(
                            n_dct=int(2**15),
                            min=-5, max=+5,
                            evaluate_at=np.linspace(-1, 1, 1e4)
                        )

                    else:
                        raise NotImplementedError(
                            'Applying KDEs to dimension "%s" not implemented.'
                            % kde_dim
                        )
    ...
    def generate_smearing_kernel(self, flavintgroup):
        ...
        """
        assert flavintgroup in self.kde_info
        #reordered_input_binning = self.input_binning.reorder_dimensions(
        #    self.kde_binning
        #)
        ...
        kernel = kernel_binning.ones(name='kernel', dtype=FTYPE)

        # 0. Work on each input dimension independently
        # 1. 

        for input_bin in product(*self.input_binning):
            input_bin_
            for dim_num, input_bin_dim in enumerate(input_bin):
                ...
                kde_info = self.kde_info[flavintgroup][dim_basename][kde_coord]

        # First figure out the fraction of events per (input) true energy bin
        # that will end up in each (output) PID bin
        if 'pid' in self.output_binning:
            self.kde_info[flavintgroup]['pid']
